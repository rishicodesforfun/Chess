...
void BoardState::do_move(Move m, StateInfo &new_st) {
  ...
  if (flags == EN_PASSANT) {
    ...
    if (to == G1) {
      remove_piece(H1);
      place_piece(W_ROOK, F1);
    } else if (to == C1) {
      remove_piece(A1);
      place_piece(W_ROOK, D1);
    } else if (to == G8) {
      remove_piece(H8);
      place_piece(B_ROOK, F8);
    } else if (to == C8) {
      remove_piece(A8);
      place_piece(B_ROOK, D8);
    }
  ...
  if (type_of(moving_piece) == PAWN &&
      std::abs(rank_of(to) - rank_of(from)) == 2) {
    new_st.ep_square =
        make_square((rank_of(from) + rank_of(to)) / 2, file_of(from));
  }

  // Update castling rights (very simplified, needs proper bitmask per square)
  if (type_of(moving_piece) == KING) {
    new_st.castling_rights &=
        (stm == WHITE) ? ~(WHITE_OO | WHITE_OOO) : ~(BLACK_OO | BLACK_OOO);
  }
